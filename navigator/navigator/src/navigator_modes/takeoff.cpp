...
#include "navigator/navigator_modes/takeoff.hpp"
                               ...
                               std::string nav_mode)
{
	...
	node->declare_parameter("takeoff.velocity_tolerance", 0.05);
...
bool Takeoff::activate(navigator_msgs::action::NavigatorCommand::Goal::ConstSharedPtr goal)
{
	auto node = node_.lock();
...
void Takeoff::publishNavigatorSetpoint()
{
	...
	odom.child_frame_id = navigator_->getVehicleName();

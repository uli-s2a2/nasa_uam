#include <qlearning_controller/qlearning_controller.hpp>
...
QLearningController::QLearningController() : rclcpp::Node("controller")
{
	computeSvecMatrix();
	...
	navigator_setpoint_sub_ =
			this->create_subscription<nav_msgs::msg::Odometry>(
					"/navigator/position_setpoint", 10,
	...
	vehicle_odometry_sub_ =
			this->create_subscription<nav_msgs::msg::Odometry>("/vehicle_interface/odometry", 10,
					[this](const nav_msgs::msg::Odometry::UniquePtr msg) {
...
//						RCLCPP_INFO(get_logger(), "time resolution: %.4f", time_resolution_);
	...
	vehicle_attitude_setpoint_pub_ =
			this->create_publisher<controller_msgs::msg::AttitudeSetpoint>("/control/attitude_setpoint", 10);
	qlearning_status_pub_ =
			this->create_publisher<qlearning_controller_msgs::msg::QLearningStatus>("/control/qlearning_status", 10);
...
{
	Vector3d thrust_des;
	controller_msgs::msg::AttitudeSetpoint attitude_setpoint;
	Vector3d x_c_des;
...
{
	qlearning_controller_msgs::msg::QLearningStatus qlearning_status;
...
int main(int argc, char *argv[])
{
	...
	rclcpp::spin(std::make_shared<controller::QLearningController>());
	rclcpp::shutdown();
	return 0;
}
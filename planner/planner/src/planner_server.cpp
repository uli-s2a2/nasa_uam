...
#include "planner/planner_server.hpp"
...
: rclcpp_lifecycle::LifecycleNode("planner_server", "", options),
  planner_loader_("planner", "planner::PlannerBase"),
  default_ids_{"rrtx_static"},
  default_types_{"rrtx_static_planner::RrtxStatic"}
{
	declare_parameter("planner_plugins", default_ids_);
	declare_parameter("global_frame", rclcpp::ParameterValue(std::string("world")));
...
PlannerServer::on_configure(const rclcpp_lifecycle::State &state)
{
	...
	for (size_t i=0; i != planner_ids_.size(); i++) {
		try{
			planner_types_[i] = util::get_plugin_type_param(node, planner_ids_[i]);
			...
			RCLCPP_FATAL(
					...
					ex.what());
			return util::CallbackReturn::FAILURE;
		}
	}
...
template<typename T>
bool PlannerServer::isServerInactive(std::unique_ptr<util::SimpleActionServer<T>> & action_server)
{
...
void PlannerServer::computePath()
{
	...
	} catch (planner::InvalidPlanner & ex) {
		...
		const std::string & planner_id)
{
	...
	if (planners_.find(planner_id) != planners_.end()) {
		...
		if (planners_.size() == 1 && planner_id.empty()) {
			...
			throw planner::InvalidPlanner("Planner id " + planner_id + " is invalid");
		}
	}
...
RCLCPP_COMPONENTS_REGISTER_NODE(planner::PlannerServer)